...
#tag WindowCode
	#tag Event
		Sub AppearanceChanged(dark As Boolean)
		  ExpenseDetailContainer(ScrollableArea1.Container).CategoryTable1.ReloadDataSource
		End Sub
	#tag EndEvent
	...
	#tag Event
		Sub Opening()
		  ...
		  if locationValue <> "" then
		    ...
		    catch i as UnsupportedOperationException
		  ...
		  DetailContainer.CurrentCategory.Text = DetailContainer.kCurrentCategory + Category
		End Sub
	#tag EndEvent
		...
		Sub UpdateExpense()
		  if expense <> nil then
		    ...
		    if DetailContainer.AmountField.Text <> "" then
		      expense.Value(expensesdb.kAmount) = DetailContainer.AmountField.Text.Trim
		    end if
		    
		    expense.Value(expensesdb.kName) = DetailContainer.NameField.Text.Trim
		    expense.Value(expensesdb.kDescription) = DetailContainer.DescriptionField.Text.trim
		    
		    if DetailContainer.CategoryTable1.SelectedRow.Right <> -1 then
		      var cell as MobileTableCellData = DetailContainer.CategoryTable1.RowCellData(DetailContainer.CategoryTable1.SelectedRow.Right)
